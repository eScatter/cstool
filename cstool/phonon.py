...
def phonon_crosssection_dual_branch(
    ...
    given the properties of a material. These properties should be given
    as quantities with units, where the unit must have the same
    dimensionality as those given here.
    ...
    def dcs_hi(mu, E):
        ...
        :param E: energy in Joules.
        :param theta: angle in radians."""
        two_branch_factor = 1. * ((n_BZ_lo + 0.5) * eps_ac_lo**2 / h_bar_w_BZ_lo) +
    ...
    def dcs(E, theta):
        ...
        g = interpolate(
            lambda E: partial(dcs_lo, m), partial(dcs_hi, m),
            h, E_BZ / 4, E_BZ)
...
def phonon_cs_fn(s: Settings):
    # TODO: add material parameter fDualBranch
    fDualBranch = True
    if fDualBranch:
        return phonon_crosssection_dual_branch(
            s.M_tot, s.rho_m, s.eps_ac, s.c_s, lattice=s.lattice,
            interpolate=log_interpolate)
    else:
